...
One of the best things about component-based frameworks is that they help to
    break apart complex UIs into smaller, manageable pieces.
...
I've created an [example project] that demonstrates each of these test cases
    in more detail, and preserves the intended testing outcomes.

## How You're Supposed to Test Svelte Components
Before you start ripping apart your application with component hacks,
...
Each component has its own `.svelte` file;
components should manage their own state and behaviour.
...
Since each component is self-contained in its own file,
...
if you're trying to build complex user interfaces, pages with many nested
    components,
or just trying to implement a fine-grained composition design pattern,
...
Compared to my other following suggestions, snippets are the most
    Svelte-recommended way to inject dependencies,
...
test('ComponentUnderTest with mocked dependency', () => {
  const MockedDependency = createRawSnippet(() => ({
    render : () => '<div style="display: content;"></div>',
    setup : (node) => {
...
However, this does not remove the element from the DOM and it may still
    interfere with CSS selectors that rely a direct parent-child relationship.
...
> [!IMPORTANT]
It appears that the snippet is only called on mount, not on any subsequent
    updates or re-renders.
...
vi.mock('./Dependency.svelte', async (originalModule) => {
  ...
  const spy = vi.fn(createRawSnippet(props => ({
    // props can be used to customize the mock behaviour
...
I've been documenting the process of developing the application, and all it's
...
[strengths and weaknesses]:
    ../../devlogs/blank/sveltekit.md#learning-to-love-sveltekit
...
[development logs]: ../../hello-world.md#committing-to-open-development
