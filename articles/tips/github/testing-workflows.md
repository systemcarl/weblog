...
> [!NOTE]
This article first discusses the philosophy and general strategies for testing
...
In my experience, however, a comprehensive CI/CD system is still seen as a
    luxury;
many developers still aspire to just have any automation in place
to build, test, and deploy their software.
Whether or not a CI/CD system is reliable is often an afterthought, if it's even
    considered at all.
I often hear CI/CD testing is disregarded
because CI/CD system requirements change rapidly to keep up with the software
    being deployed.
Testing also adds little value beyond mitigating risk,
which can be a hard sell when the risk is not appreciated,
    or when there are other more pressing priorities.
It's usually cheaper to just pay someone to oversee the automation and fix any
    problems as they arise — or at least some may think.

Now, even when testing is considered, there is seldom any support for testing
...
These files still define the sequence of operations to be performed
    (even if they don't themselves implement it),
    and ideally, the outcome should still be validated against the intent.
There are tools to run GitHub Actions workflows on a local machine,
like [Act],
...
## Deployment Testing
The most straightforward way to test [GitHub Actions] workflows (or any other
    CI/CD workflows) is to run them in a staging or test environment,
...
Primarily, there is often a cost (either monetary or time) to maintaining
    dedicated production-like environments for testing.
...
Since GitHub Actions workflows either directly or indirectly execute scripts
    and commands,
the resulting command line output can be used to inspect and validate the
...
However, even deployment workflows still be tested this way,
if the normally-destructive commands can be executed in a non-destructive
    manner.
A common pattern is to have a command line flag instruct the command
...
For examples of self-reporting scripts, you can check out
...
### Re-Using Workflows
In the example above, the script output is sent directly to the
    [workflow logs],
...
However, GitHub Actions does not provide ubiquitous access to the workflow logs
    during execution;
it is not easy to use these logs for validation later in the workflow.
Even if the output is captured by the runner I/O streams,
the environment variable or file the output is stored in is isolated to the
    virtual environment of the respective job.
...
The output values are defined as variables,
...
However, since any number commands or scripts can be wrapped into a single
...
[the CI/CD workflows for my personal website] also use these strategies to
    thoroughly [test all of the testing and deployment workflows]
...
The environment validation jobs also check that the environment used for test
...
[by validating workflow output]: #validating-workflow-output
[by inspecting workflow-generated artifacts]: #validating-generated-artifacts
[an example project]: https://github.com/systemcarl/github-workflow-test
