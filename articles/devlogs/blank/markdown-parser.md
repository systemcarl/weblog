...
(identifying labels used to target specific elements for styling or scripting)
...
```
If there were, HTML tags could be mapped to the appropriate theme classes
    (*i.e.* the HTML class names generated by the theme system) in the CSS,
...
Alas, this is not the case, and the HTML output of the parser needs to include
    the HTML classes matching the theme classes.
